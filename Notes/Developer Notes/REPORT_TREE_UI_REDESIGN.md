...
**Date:** 2026-02-12 (Revised 2026-02-16)
...
**Internationalization (i18n):** The root node display names "System" and "User"
must be translatable using `QObject::tr()`. The `name` column in the database stores
the untranslated English key (`"System"`, `"User"`); the UI renders the translated
text via `tr("System")` and `tr("User")`. Folder and report names within the System
branch are loaded from the `system_reports.orf` file. For localized builds, a
translated copy of the .orf file can be provided (e.g., `system_reports_de.orf`),
or the names in the master file can be translated directly since they are
developer-editable text.
...
4. Drop the old `reports` and `report_contents` tables.

**Note:** No migration of existing user reports from the old tables is required.
Users will start fresh with the User branch empty after the upgrade.
...
- **Accessible without open profile:** The File menu item for "Reports and CSV Export"
  is always enabled, even when no profile is currently open. The dialog opens and
  displays the report tree for browsing, editing, and management. The Profile combo
  box (see Section 4.4) allows the user to select any profile. If no profile is open
  when the dialog launches, the Profile combo defaults to the first available profile
  (or shows "(No profiles)" if the database has none). Export requires a valid profile
  selection.
...
+--------------------------------------------------------------------+
|                          |                                          |
|   Report Tree            |   Profile: [Guy___________________ v]   |
|   ==================     |                                          |
|   > System               |   Quick Range: [Most Recent Day    v]   |
|     > Daily Summaries    |                                          |
|       by Day             |   Dates:  Start: [01/15/2026]           |
|       by Month           |           End:   [02/12/2026]           |
|       by Week            |                                          |
|     > Other              |   ----------------------------------------|
|       Channels Used      |                                          |
|       Device Settings    |   Filename: [__________________________] |
|       Profiles           |            [...] (browse button)         |
|       Profiles with Data |                                          |
|     > Session Summaries  |   ----------------------------------------|
|       by Day             |                                          |
|       by Month           |   [============================] 0%     |
...
|       by Week            |   ----------------------------------------|
|     > Statistics         |                                          |
|       Respiratory Events |   [x] Open file after export             |
|       Sessions           |   Program: [Excel____________ v]         |
|   v User                 |                                          |
|     > SpO2               |   ----------------------------------------|
|       > my stuff         |                                          |
|         SpO2 plot        |   [Export CSV] [Edit SQL] [Cancel]       |
|         SpO2 statistics  |                                          |
|                          +------------------------------------------+
+--------------------------+
```
...
1. **Profile** combo box (`QComboBox`) — populated with all profile usernames
   from the `profiles` table. The `#PROFILE_ID` macro is substituted with
   the ID of the selected profile.
   - Default: the currently open profile (if one is open), otherwise the
     first profile in the list.
   - If no profiles exist, shows "(No profiles)" and Export CSV is disabled.
   - Changing the profile updates the calendar color-coding to reflect
     that profile's data dates.
   - The profile combo is populated by querying `SELECT id, username FROM profiles
     ORDER BY username`. The combo stores the profile ID as item data
     (`QComboBox::addItem(username, profileId)`).

2. **Quick Range** combo box (same options as current: Most Recent Day, Last Week,
   Last Fortnight, Last Month, Last 6 Months, Last Year, Everything, Custom).

3. **Date range** with Start and End `QDateEdit` controls with calendar popups.
   Calendar days are color-coded for CPAP/Oximetry data presence (same as current,
   but reflecting the profile selected in the Profile combo above).

4. **Filename** line edit with browse button (same as current).

5. **Progress bar** (same as current).

6. **Open after export** section:
   - **Checkbox:** `[x] Open file after export` — when checked, the exported CSV
     file is automatically opened with the selected program after export completes.
   - **Program combo box:** `QComboBox` with editable text, allowing the user to
     type a program path or select from presets. Presets include:
     - `(System default)` — uses `QDesktopServices::openUrl()` to open the CSV
       with whatever program the OS associates with .csv files.
     - `Excel` — launches Microsoft Excel with the CSV file as argument.
     - `LibreOffice Calc` — launches LibreOffice Calc.
     - `Custom...` — user types or browses for a custom executable path.
   - The checkbox state, selected program, and custom path are persisted in
     `QSettings` under `ReportExporter/openAfterExport`,
     `ReportExporter/openProgram`, and `ReportExporter/customProgramPath`.
   - Implementation: after CSV write completes successfully, if the checkbox is
     checked, launch the selected program via `QProcess::startDetached()` (for
     named programs) or `QDesktopServices::openUrl()` (for system default).

7. **Button row:** Export CSV, Edit SQL, Cancel.
...
### 4.4.1 Remember Last Report

The dialog remembers and restores the last selected report between sessions:

- When the user selects a report in the tree, its database `id` is saved to
  `QSettings` under `ReportExporter/lastReportId`.
- When the dialog opens, the tree is loaded and the previously selected report
  is re-selected and scrolled into view. If the saved ID no longer exists
  (e.g., deleted or system reports rebuilt), no selection is made.
- The last-used filename is also saved/restored via `QSettings` under
  `ReportExporter/lastFilename`.
- The last-used profile is saved/restored via `QSettings` under
  `ReportExporter/lastProfileId`. If a profile is currently open, it takes
  precedence over the saved value.
- The tree expand/collapse state is saved/restored via `QSettings` under
  `ReportExporter/treeState` (using `QTreeView::saveState()` equivalent —
  store the list of expanded node IDs).

### 4.5 Button Behavior

| Button | Behavior |
|--------|----------|
| **Export CSV** | Runs the selected report's SQL query (with macro substitution using the selected Profile) and writes results to CSV file. If "Open file after export" is checked, launches the configured program with the CSV file. Enabled only when a report leaf is selected AND a filename is set AND a valid profile is selected. |
...
Use `QStyle::standardIcon()` from Qt for all tree node icons. No custom icon
files are needed at this time.

| Node Type | QStyle Icon | Description |
|-----------|-------------|-------------|
| System root | `SP_DriveNetIcon` or `SP_ComputerIcon` | Indicates protected/read-only branch |
| User root | `SP_DirHomeIcon` | Indicates user-customizable branch |
| Folder (system) | `SP_DirIcon` | Read-only organizational folder |
| Folder (user) | `SP_DirOpenIcon` / `SP_DirIcon` | User-created folder (open/closed) |
| Report (system) | `SP_FileIcon` | Read-only system report |
| Report (user) | `SP_FileDialogDetailedView` | User-editable report |

System nodes should be rendered with a subtle visual distinction (e.g., slightly
gray text via `QStandardItem::setForeground(Qt::darkGray)`) to reinforce the
read-only nature. Custom icons from `:/icons/` or `Resources.qrc` can be
substituted later if the standard icons prove insufficient.
...
private:
    ...
    QModelIndex findNodeById(qint64 nodeId) const;

    // Tree state persistence
    QList<qint64> getExpandedNodeIds() const;
    void restoreExpandedNodes(const QList<qint64> &nodeIds);
...
6. Drop old `reports` and `report_contents` tables (no user data migration needed).
...
1. Create `reportexporter.ui` in Qt Designer with the layout from Section 4.2:
   ...
   - Profile combo box at top of right panel.
   - Date range controls, filename controls, progress bar, buttons.
   - "Open file after export" checkbox and program combo at bottom.
2. Create `reportexporter.h` and `.cpp`:
   - Constructor loads tree model, populates profile combo, sets up date range,
     connects signals, restores saved settings from QSettings.
   - Profile combo populated from `profiles` table; defaults to currently open
     profile (if any) or last-used profile.
   - Single-click selects report, updates status label and button states.
   - Export CSV button runs query with macro substitution (using selected profile)
     and writes CSV. On success, optionally launches post-export program.
   - Edit SQL button opens SQLEditor (read-only for system, editable for user).
   - Cancel button saves settings to QSettings and closes dialog.
3. Port the CSV export logic from `exportcsv.cpp` (substituteMacros, query execution,
   CSV writing, progress bar updates).
4. Port the calendar color-coding logic from `exportcsv.cpp`, updating colors when
   profile selection changes.
5. Implement `launchPostExportProgram()` — uses `QDesktopServices::openUrl()` for
   system default or `QProcess::startDetached()` for named programs.
6. Implement `saveSettings()` / `restoreSettings()` — persist last report ID,
   last filename, last profile ID, tree expand state, open-after-export checkbox,
   and selected program to QSettings.
7. Update `mainwindow.cpp` to launch `ReportExporter` instead of `ExportCSV`.
   Make the menu item always enabled (not dependent on profile being open).
8. Add new files to `oscar.pro`.
9. Manual test: Open dialog, browse tree, select reports, change profile,
   export CSV, verify output, verify post-export launch, verify settings persist.

**Deliverables:** Working replacement for ExportCSV with tree-based selection,
profile selection, post-export launch, and settings persistence.
...
3. Confirm old database tables (`reports`, `report_contents`) were dropped in Phase 1.
   Remove any remaining references to them in schema code.
...
7. Test full workflow end-to-end:
   ...
   - Upgrade from old version: old tables dropped cleanly, system reports rebuilt.
   - All tree operations work.
   - CSV export produces correct output.
   - Profile selection works (including with no profile open).
   - Post-export program launch works (system default and named programs).
   - Settings persistence works (last report, last profile, tree state, etc.).
...
These questions were raised during the design phase and have been resolved:

| # | Question | Resolution |
|---|----------|------------|
| 1 | **Icon assets:** Custom icons or Qt standard icons? | **Use `QStyle::standardIcon()` for now.** See Section 4.7 for specific icon assignments. Custom icons can be added later if needed. |
| 2 | **Multi-report CSV export:** Single file or multiple files? | **Deferred.** Single-report export only for now. Multi-report batch export is a future enhancement. |
| 3 | **Profiles and Profiles-with-data reports:** Missing from system reports. | **Done.** Added to `system_reports.orf` under the Other folder. |
| 4 | **Undo support:** Should tree operations support Ctrl+Z? | **Deferred.** Future enhancement. Confirmation dialogs provide sufficient safety net for now. |
| 5 | **Tree state persistence:** Save expand/collapse state between dialog opens? | **Yes.** Save to QSettings. See Section 4.4.1 for details. |
