/* ExportCSV module implementation — Version 1 (1.7.x-style) CSV export
 *
    ...
    ui(new Ui::ExportCSV)
{
    ...
    // Connect signals to update which days have CPAP data when the month is changed
...
void ExportCSV::on_exportButton_clicked()
{
    ...
    EventDataType percent = percentile;
    ...
    if (ui->rb1_details->isChecked()) {
        ...
        for (int i = 0; i < avglist.size(); i++) {
            header += sep + Day::calcMiddleLabel(avglist[i]);
        }
        ...
        for (int i = 0; i < maxlist.size(); i++) {
            header += sep + Day::calcMaxLabel(maxlist[i]);
        }
    }
        ...
        if (day) {
            ...
            if (ui->rb1_Summary->isChecked()) {
                ...
                for (int i = 0; i < avglist.size(); i++) {
                    ...
                    data += sep + QString::number(avg);
                }

                for (int i = 0; i < p90list.size(); i++) {
                    float p90 = day->percentile(p90list.at(i), percent);
                    data += sep + QString::number(p90);
                }

                for (int i = 0; i < maxlist.size(); i++) {
                    float max = day->calcMax(maxlist.at(i));
                    data += sep + QString::number(max);
                }
                ...
                for (int i = 0; i < day->size(); i++) {
                    ...
                    for (int j = 0; j < avglist.size(); j++) {
                        data += sep + QString::number(sess->calcMiddle(avglist.at(j)));
                    }

                    for (int j = 0; j < p90list.size(); j++) {
                        data += sep + QString::number(sess->percentile(p90list.at(j), percent));
                    }

                    for (int i = 0; i < maxlist.size(); i++) {
                        data += sep + QString::number(sess->calcMax(maxlist.at(i)));
                    }
