...
def main():
    ...
    print(sep)
    print(fmt.format("File", "Messages", "Unfinished", "Obsolete", "Vanished", "Active"))
    print(sep)

    tot_msg = tot_unf = tot_obs = tot_van = 0

    for fname in files:
        msg, unf, obs, van = analyse(os.path.join(directory, fname))
        active = msg - obs - van
        tot_msg += msg
        tot_unf += unf
        tot_obs += obs
        ...
        print(fmt.format(name, msg, unf, obs, van, active))

    print(sep)
    tot_active = tot_msg - tot_obs - tot_van
    print(fmt.format(f"TOTAL ({len(files)} files)", tot_msg, tot_unf, tot_obs, tot_van, tot_active))
    print()
